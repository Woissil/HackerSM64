...
#define OR_LONG(field, value) BC_BB(BHV_CMD_OR_LONG, field), BC_W(value)

// Performs a bit clear with the specified short. Unused in favor of the 32-bit version.
#define BIT_CLEAR(field, value) BC_BBH(BHV_CMD_BIT_CLEAR, field, value)
...
#define SET_INT_RAND_RSHIFT(field, min, rshift)                                                        \
    BC_BBH(BHV_CMD_SET_INT_RAND_RSHIFT, field, min), BC_H(rshift)

// Sets the specified field to a random float in the given range.
#define SET_RANDOM_FLOAT(field, min, range) BC_BBH(BHV_CMD_SET_RANDOM_FLOAT, field, min), BC_H(range)

// Sets the specified field to a random integer in the given range.
#define SET_RANDOM_INT(field, min, range) BC_BBH(BHV_CMD_SET_RANDOM_INT, field, min), BC_H(range)

// Adds a random float in the given range to the specified field.
#define ADD_RANDOM_FLOAT(field, min, range) BC_BBH(BHV_CMD_ADD_RANDOM_FLOAT, field, min), BC_H(range)
...
#define ADD_INT_RAND_RSHIFT(field, min, rshift)                                                        \
    BC_BBH(BHV_CMD_ADD_INT_RAND_RSHIFT, field, min), BC_H(rshift)

// No operation. Unused.
#define CMD_NOP_1(field) BC_BB(BHV_CMD_NOP_1, field)

// No operation. Unused.
#define CMD_NOP_2(field) BC_BB(BHV_CMD_NOP_2, field)

// Sets the current model ID of the object.
#define SET_MODEL(modelID) BC_B0H(BHV_CMD_SET_MODEL, modelID)

// Spawns a child object with the specified model and behavior.
#define SPAWN_CHILD(modelID, behavior) BC_B(BHV_CMD_SPAWN_CHILD), BC_W(modelID), BC_PTR(behavior)

// Exits the behavior script and despawns the object.
// Often used to end behavior scripts that do not contain an infinite loop.
#define DEACTIVATE() BC_B(BHV_CMD_DEACTIVATE)

// Finds the floor triangle directly under the object and moves the object down to it.
#define DROP_TO_FLOOR() BC_B(BHV_CMD_DROP_TO_FLOOR)
...
#define SUM_INT(fieldDst, fieldSrc1, fieldSrc2) BC_BBBB(BHV_CMD_SUM_INT, fieldDst, fieldSrc1, fieldSrc2)

// Billboards the current object, making it always face the camera.
#define BILLBOARD() BC_B(BHV_CMD_BILLBOARD)

// Hides the current object.
#define HIDE() BC_B(BHV_CMD_HIDE)

// Sets the size of the object's cylindrical hitbox.
#define SET_HITBOX(radius, height) BC_B(BHV_CMD_SET_HITBOX), BC_HH(radius, height)

// No operation. Unused.
#define CMD_NOP_4(field, value) BC_BBH(BHV_CMD_NOP_4, field, value)

// Delays the behavior script for the number of frames given by the value of the specified field.
#define DELAY_VAR(field) BC_BB(BHV_CMD_DELAY_VAR, field)
...
#define BEGIN_REPEAT_UNUSED(count) BC_BB(BHV_CMD_BEGIN_REPEAT_UNUSED, count)

// Loads the animations for the object. <field> is always set to oAnimations.
#define LOAD_ANIMATIONS(field, anims) BC_BB(BHV_CMD_LOAD_ANIMATIONS, field), BC_PTR(anims)

// Begins animation and sets the object's current animation index to the specified value.
#define ANIMATE(animIndex) BC_BB(BHV_CMD_ANIMATE, animIndex)
...
#define SPAWN_CHILD_WITH_PARAM(bhvParam, modelID, behavior)                                            \
    BC_B0H(BHV_CMD_SPAWN_CHILD_WITH_PARam, bhvParam), BC_W(modelID), BC_PTR(behavior)

// Loads collision data for the object.
#define LOAD_COLLISION_DATA(collisionData) BC_B(BHV_CMD_LOAD_COLLISION_DATA), BC_PTR(collisionData)
...
#define SET_HITBOX_WITH_OFFSET(radius, height, downOffset)                                             \
    BC_B(BHV_CMD_SET_HITBOX_WITH_OFFSet), BC_HH(radius, height), BC_H(downOffset)

// Spawns a new object with the specified model and behavior.
#define SPAWN_OBJ(modelID, behavior) BC_B(BHV_CMD_SPAWN_OBJ), BC_W(modelID), BC_PTR(behavior)

// Sets the home position of the object to its current position.
#define SET_HOME() BC_B(BHV_CMD_SET_HOME)

// Sets the size of the object's cylindrical hurtbox.
#define SET_HURTBOX(radius, height) BC_B(BHV_CMD_SET_HURTBOX), BC_HH(radius, height)

// Sets the object's interaction type.
#define SET_INTERACT_TYPE(type) BC_B(BHV_CMD_SET_INTERACT_TYPE), BC_W(type)
...
#define SET_OBJ_PHYSICS(wallHitboxRadius, gravity, bounciness, dragStrength, friction, buoyancy,       \
                        unused1, unused2)                                                              \
    BC_B(BHV_CMD_SET_OBJ_PHYSICS), BC_HH(wallHitboxRadius, gravity), BC_HH(bounciness, dragStrength),  \
        BC_HH(friction, buoyancy), BC_HH(unused1, unused2)

// Sets the object's interaction subtype. Unused.
#define SET_INTERACT_SUBTYPE(subtype) BC_B(BHV_CMD_SET_INTERACT_SUBTYPE), BC_W(subtype)

// Sets the object's size to the specified percentage.
#define SCALE(unusedField, percent) BC_BBH(BHV_CMD_SCALE, unusedField, percent)
...
#define PARENT_BIT_CLEAR(field, flags) BC_BB(BHV_CMD_PARENT_BIT_CLEAR, field), BC_W(flags)

// Animates an object using texture animation. <field> is always set to oAnimState.
#define ANIMATE_TEXTURE(field, rate) BC_BBH(BHV_CMD_ANIMATE_TEXTURE, field, rate)

// Disables rendering for the object.
#define DISABLE_RENDERING() BC_B(BHV_CMD_DISABLE_RENDERING)

// Unused. Sets the specified field to an integer. Wastes 4 bytes of space for no reason at all.
#define SET_INT_UNUSED(field, value) BC_BB(BHV_CMD_SET_INT_UNUSED, field), BC_HH(0, value)

// Spawns a water droplet with the given parameters.
#define SPAWN_WATER_DROPLET(dropletParams) BC_BPTR(BHV_CMD_SPAWN_WATER_DROPLET, dropletParams)
...
const BehaviorScript bhvMrIIris[] = {
    ...
    CALL_NATIVE(bhv_mr_i_iris_loop), END_LOOP(),
};
...
const BehaviorScript bhvKoopaShellUnderwater[] = {
    BEGIN(OBJ_LIST_GENACTOR),
    OR_LONG(oFlags, (OBJ_FLAG_HOLDABLE | OBJ_FLAG_COMPUTE_DIST_TO_MARIO
                     | OBJ_FLAG_UPDATE_GFX_POS_AND_ANGLE | OBJ_FLAG_SILHOUETTE)),
    BEGIN_LOOP(),
...
const BehaviorScript bhvSquishablePlatform[] = {
    BEGIN(OBJ_LIST_SURFACE),
    OR_LONG(oFlags, (OBJ_FLAG_COMPUTE_DIST_TO_MARIO | OBJ_FLAG_SET_FACE_YAW_TO_MOVE_YAW
                     | OBJ_FLAG_UPDATE_GFX_POS_AND_ANGLE | OBJ_FLAG_DONT_CALC_COLL_DIST)),
...
const BehaviorScript bhvKoopaShell[] = {
    ...
    OR_LONG(oFlags,
            (OBJ_FLAG_UPDATE_GFX_POS_AND_ANGLE
             | OBJ_FLAG_SILHOUETTE)), //! Silhouette doesn't show up in-game, due to combiner modes.
    SET_OBJ_PHYSICS(/*Wall hitbox radius*/ 30, /*Gravity*/ -400, /*Bounciness*/ -50,
                    /*Drag strength*/ 1000, /*Friction*/ 1000, /*Buoyancy*/ 200, /*Unused*/ 0, 0),
    BEGIN_LOOP(),
...
const BehaviorScript bhvMenuButtonManager[] = {
    BEGIN(OBJ_LIST_LEVEL),
    OR_INT(oFlags, (OBJ_FLAG_SET_THROW_MATRIX_FROM_TRANSFORM
                    | OBJ_FLAG_UPDATE_TRANSFORM_FOR_THROW_MATRIX | OBJ_FLAG_UPDATE_GFX_POS_AND_ANGLE)),
...
const BehaviorScript bhvStar[] = {
    ...
    SET_FLOAT(oDrawingDistance, 22000),
    BEGIN_LOOP(),
...
const BehaviorScript bhvFloombaStartup[] = {
    ...
    OR_INT(oFlags, OBJ_FLAG_ACTIVE_FROM_AFAR | OBJ_FLAG_SET_FACE_YAW_TO_MOVE_YAW
                       | OBJ_FLAG_UPDATE_GFX_POS_AND_ANGLE),
    LOAD_ANIMATIONS(oAnimations, goomba_seg8_anims_0801DA4C),
    SET_HOME(),
    CALL_NATIVE(bhv_goomba_init),
    ...
    SET_INT(oIntangibleTimer, -1),
    SET_OBJ_PHYSICS(/*Wall hitbox radius*/ 0, /*Gravity*/ 0, /*Bounciness*/ 0, /*Drag strength*/ 1000,
                    /*Friction*/ 1000, /*Buoyancy*/ 0, /*Unused*/ 0, 0),
    BEGIN_LOOP(),
    CALL_NATIVE(bhv_goomba_update),
    END_LOOP(),
};
...
const BehaviorScript bhvWoodenPost[] = {
    ...
    OR_LONG(oFlags, (OBJ_FLAG_COMPUTE_ANGLE_TO_MARIO | OBJ_FLAG_COMPUTE_DIST_TO_MARIO
                     | OBJ_FLAG_UPDATE_GFX_POS_AND_ANGLE | OBJ_FLAG_DONT_CALC_COLL_DIST)),
    SET_OBJ_PHYSICS(/*Wall hitbox radius*/ 0, /*Gravity*/ -400, /*Bounciness*/ -50,
                    /*Drag strength*/ 1000, /*Friction*/ 1000, /*Buoyancy*/ 200, /*Unused*/ 0, 0),
    SET_INT(oNumLootCoins, 5),
...
const BehaviorScript bhvRacingPenguin[] = {
    ...
    LOAD_ANIMATIONS(oAnimations, penguin_seg5_anims_05008B74),
    ANIMATE(PENGUIN_ANIM_IDLE),
    SET_OBJ_PHYSICS(/*Wall hitbox radius*/ 200, /*Gravity*/ -800, /*Bounciness*/ -5,
                    /*Drag strength*/ 0, /*Friction*/ 0, /*Buoyancy*/ 0, /*Unused*/ 0, 0),
    SCALE(/*Unused*/ 0, /*Field*/ 400),
